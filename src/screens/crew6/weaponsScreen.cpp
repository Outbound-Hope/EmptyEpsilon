...
: GuiOverlay(owner, "WEAPONS_SCREEN", colorConfig.background), direction(direction)
{
    ...
    radar->setCallbacks(
        [this](sf::Vector2f position) {
            ...
            if (my_spaceship){
                this->direction == RearAndLeft?  my_spaceship->commandSetRearTarget(targets.get()) : my_spaceship->commandSetTarget(targets.get());
            }
        }, nullptr, nullptr
    );
    ...
    if (gameGlobalInfo->use_beam_shield_frequencies || gameGlobalInfo->use_system_damage)
    {
        ...
        (new GuiLabel(beam_info_box, "BEAM_INFO_LABEL", "Railgun Specs", 30))->addBackground()->setSize(GuiElement::GuiSizeMax, 50);
        (new GuiPowerDamageIndicator(beam_info_box, "", SYS_BeamWeapons, ACenterLeft))->setSize(GuiElement::GuiSizeMax, 50);
        (new GuiBeamFrequencySelector(beam_info_box, "BEAM_FREQUENCY_SELECTOR", direction))->setPosition(0, 0, ABottomRight)->setSize(GuiElement::GuiSizeMax, 50);
        (new GuiBeamTargetSelector(beam_info_box, "BEAM_TARGET_SELECTOR", direction))->setPosition(0, -50, ABottomRight)->setSize(GuiElement::GuiSizeMax, 50);
